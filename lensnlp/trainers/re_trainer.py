...
from utilis.data import Sentence, Token, MultiCorpus, Corpus
from models import TextClassifier, SequenceTagger,RelationExtraction
from utilis.training_utils import Metric, init_output_file, clear_embeddings, EvaluationMetric, \
    log_line, init_log, WeightExtractor

from hyper_parameters import Parameter
...
class ModelTrainer:
    ...
    def train(self,
              ...
              evaluation_metric: EvaluationMetric = EvaluationMetric.MACRO_ACCURACY,
    ...
    def final_test(self,
        ...
        if (base_path / 'best-model.pt').exists():
            ...
            if isinstance(self.model,RelationExtraction):
                self.model = RelationExtraction.load_from_file(base_path / 'best-model.pt')
        ...
        log.info(f'acc{test_metric.macro_avg_accuracy()}')
    ...
    def evaluate(model: nn.Model, data_set: List[Sentence],
        ...
        if isinstance(model, TextClassifier):
            ...
            return ModelTrainer._evaluate_sequence_tagger(model, data_set, eval_mini_batch_size, embeddings_in_memory,
                                                          out_path)
        elif isinstance(model,RelationExtraction):
            return ModelTrainer._evaluate_relation_extraction(model, data_set, eval_mini_batch_size, embeddings_in_memory,
                                                          out_path)
    @staticmethod
    ...
    @staticmethod
    def _evaluate_relation_extraction(model: nn.Model,
                                  sentences: List[Sentence],
                                  eval_mini_batch_size: int = 32,
    ...
    def load_from_checkpoint(checkpoint_file: Path, model_type: str, corpus: Corpus, optimizer: Optimizer = SGD):
        ...
        if model_type == 'TextClassifier':
            ...
            return ModelTrainer(checkpoint['model'], corpus, optimizer, epoch=checkpoint['epoch'],
                                loss=checkpoint['loss'], optimizer_state=checkpoint['optimizer_state_dict'],
                                scheduler_state=checkpoint['scheduler_state_dict'])
        if model_type == 'RelationExtraction':
            checkpoint = RelationExtraction.load_checkpoint(checkpoint_file)
            return ModelTrainer(checkpoint['model'], corpus, optimizer, epoch=checkpoint['epoch'],
                                loss=checkpoint['loss'], optimizer_state=checkpoint['optimizer_state_dict'],
                                scheduler_state=checkpoint['scheduler_state_dict'])

        raise ValueError('Incorrect model type! Use one of the following: "SequenceTagger", "TextClassifier","RelationExtraction".')
